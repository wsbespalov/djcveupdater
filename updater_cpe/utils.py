...
def upload_file():
    ...
    if not os.path.isdir(os.path.join(LOCAL_BASE_DIR, CPEConfig.file_storage_root)):
        os.mkdir(os.path.join(LOCAL_BASE_DIR, CPEConfig.file_storage_root))
    try:
        file_path = os.path.join(os.path.join(LOCAL_BASE_DIR, CPEConfig.file_storage_root), CPEConfig.cpe_file)
        head = requests.head(CPEConfig.source)
        ...
        file = requests.get(CPEConfig.source, stream=True)
